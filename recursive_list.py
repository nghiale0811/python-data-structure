...
class RecursiveList:
    ...
    def count(self, item: Any) -> int:
        ...
        if self.is_empty():
            return 0
        elif self._first == item and self._rest.is_empty():
            return 1
        else:
            result = 0
            if self._first == item:
                result = 1
            result += self._rest.count(item)
            return result
    ...
    def insert(self, index: int, item: Any) -> None:
        ...
        if index == 0:
            ...
            while i < index:
                ...
                i += 1
            tmp = RecursiveList([])
            tmp._first = curr._first
            tmp._rest = curr._rest
            curr._first = item
            curr._rest = tmp
        elif index == len(self):
            curr = self
