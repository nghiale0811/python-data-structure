...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        >>> bst.height()
        2
        >>> bst._right._right = BinarySearchTree(9)
        >>> bst.height()
        3
        >>> bst._right._left = BinarySearchTree(9)
        >>> bst.height()
        3
        """
        if self.is_empty():
            ...
            if not self._left.is_empty():
                cnt1 += self._left.height()
            if not self._right.is_empty():
                cnt2 += self._right.height()
            cnt = max(cnt1, cnt2)
        return cnt
    ...
    def rotate_right(self) -> None:
        ...
        """
        # TODO: implement this method
        rs = BinarySearchTree(self._root)
        rs._left = self._left._right
        rs._right = self._right
    ...
    def rotate_left(self) -> None:
        ...
        """
        # TODO: implement this method!
        ls = BinarySearchTree(self._root)
        ls._left = self._left
        ls._right = self._right._left
    ...
    def closest(self, item: int) -> Optional[int]:
        ...
        By "closest" here, we mean the value that minimizes the absolute
        distance to <item>.
        If there is a tie, return the smaller value.
        Return None is this BST is empty.
        ...
        if self.is_empty():
            ...
            right_closest = self._left.closest(item)
            if right_closest is not None and \
                    item - right_closest <= self._root - item:
                return right_closest
            else:
                return self._root

    def print_all(self) -> None:
        """Print all the elements in this bst in level order.
        >>> bst = BinarySearchTree(7)
        >>> left = BinarySearchTree(3)
        >>> left._left = BinarySearchTree(2)
        >>> left._right = BinarySearchTree(5)
        ...
        if self.is_empty():
            return
        q = [self]  # q.pop(0) for removing, and q.append for adding
        while len(q) != 0:
            item = q.pop(0)
            print(item._root)
